...
pub fn test(text: String) -> String {
    ...
    let str = "Hello from Rust! You said: ".to_string();

   
    // let mut array = vec!["Hello".to_string(), "from".to_string(), "Rust".to_string()];
    // print_array(&array);
    // array.push("Some".to_string());
...
fn print_text(text: String) {
    let text_new = text + "!";
    println!("Hello from Rust! You said: {}", text_new);
}
